import buenardWoff2 from '@fontsource-variable/buenard/files/buenard-latin-wght-normal.woff2?url'
import interWoff2 from '@fontsource-variable/inter/files/inter-latin-wght-normal.woff2?url'

/**
 * The two weight-variable latin subsets, preloaded so body copy does not reflow
 * after first paint. Separate from `assets.ts` because `?url` only resolves
 * under Vite; see the note there.
 */
export const FONT_PRELOAD_LINKS = [interWoff2, buenardWoff2].map(
  (href) =>
    ({
      rel: 'preload',
      href,
      as: 'font',
      type: 'font/woff2',
      crossOrigin: 'anonymous',
    }) as const,
)
